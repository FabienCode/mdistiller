...
def edu_heat(stu_feat, tea_feat, selected_indices):
    dkd_stu_feat_flatten = stu_feat.reshape(stu_feat.shape[0], -1)[selected_indices, :]
    dkd_tea_feat_flatten = tea_feat.reshape(tea_feat.shape[0], -1)[selected_indices, :]
    ...
    edu_similarity = np.sqrt(np.sum((dkd_stu_feat_flatten_pca - dkd_tea_feat_flatten_pca)**2, axis=1))
    ratio = len(np.where(edu_similarity > 0)[0])/edu_similarity.size
    return edu_similarity, ratio
