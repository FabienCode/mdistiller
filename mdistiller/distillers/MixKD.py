...
def aug_feat(feature_weak, feature_strong, region_w, region_s):
    ...
    for batch_idx in range(b):
        ...
        w_ys, w_xs = torch.meshgrid(torch.arange(w_y1, w_y2 + 1), torch.arange(w_x1, w_x2 + 1), indexing='ij')
        s_ys, s_xs = torch.meshgrid(torch.arange(s_y1, s_y2 + 1), torch.arange(s_x1, s_x2 + 1), indexing='ij')
