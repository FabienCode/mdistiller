...
class KD_strong(Distiller):
    ...
    def __init__(self, student, teacher, cfg):
        ...
        self.conv_reg_weak = ConvReg(
            feat_s_shapes[self.hint_layer], feat_t_shapes[self.hint_layer]
        )
        self.conv_reg_strong = ConvReg(
            feat_s_shapes[self.hint_layer], feat_t_shapes[self.hint_layer]
        )

    def get_learnable_parameters(self):
        return super().get_learnable_parameters() + list(self.conv_reg_weak.parameters()) + list(self.conv_reg_strong.parameters())

    def get_extra_parameters(self):
        num_p = 0
        for p in self.conv_reg_weak.parameters():
            num_p += p.numel()
        for p in self.conv_reg_strong.parameters():
            num_p += p.numel()
        return num_p
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        f_s_weak = self.conv_reg_weak(feature_student_weak["feats"][self.hint_layer])
        loss_feat_weak = self.feat_loss_weight * F.mse_loss(
            f_s_weak, feature_teacher_weak["feats"][self.hint_layer]
        )
        f_s_strong = self.conv_reg_strong(feature_student_strong["feats"][self.hint_layer])
