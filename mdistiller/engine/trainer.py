...
class BaseTrainer_addloss_info(object):
    def __init__(self, experiment_name, distiller, train_loader, val_loader, cfg):
        self.cfg = cfg
        self.distiller = distiller
        self.train_loader = train_loader
    ...
    def train_epoch(self, epoch):
        ...
        train_meters = {
            ...
            "losses": AverageMeter(),
            "loss_ce": AverageMeter(),
            "loss_fitnet": AverageMeter(),
            "loss_logits": AverageMeter(),
            "top1": AverageMeter(),
            "top5": AverageMeter(),
        }
        num_iter = len(self.train_loader)
    ...
    def train_iter(self, data, epoch, train_meters):
        ...
        # backward
        loss_ce = losses_dict["loss_ce"]
        loss_fitnet = losses_dict["loss_feat"]
        loss_logits = losses_dict["loss_kd"]
        loss = sum([l.mean() for l in losses_dict.values()])
        loss.backward()
        self.optimizer.step()
        train_meters["training_time"].update(time.time() - train_start_time)
        ...
        train_meters["losses"].update(loss.cpu().detach().numpy().mean(), batch_size)
        train_meters["loss_ce"].update(loss_ce.cpu().detach().numpy().mean(), batch_size)
        train_meters["loss_fitnet"].update(loss_fitnet.cpu().detach().numpy().mean(), batch_size)
        train_meters["loss_logits"].update(loss_logits.cpu().detach().numpy().mean(), batch_size)
        train_meters["top1"].update(acc1[0], batch_size)
        train_meters["top5"].update(acc5[0], batch_size)
        # print info
        msg = "Epoch:{}| Time(data):{:.3f}| Time(train):{:.3f}| Loss:{:.4f}| Loss_task:{:.4f}| Loss_fitnet:{:.4f}| Loss_logits:{:.4f}| Top-1:{:.3f}| Top-5:{:.3f}".format(
            epoch,
            train_meters["data_time"].avg,
            train_meters["training_time"].avg,
            train_meters["losses"].avg,
            train_meters["loss_ce"].avg,
            train_meters["loss_fitnet"].avg,
            train_meters["loss_logits"].avg,
            train_meters["top1"].avg,
            train_meters["top5"].avg,
        )
        return msg
